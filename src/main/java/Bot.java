...
/*
1. Понять как делать и что такое аннотации для чего они используются
2. Закрепить понятия рефлекции
3. Прочитать код повторно и понять полность
4. Что такое инстанс зачем оно нужно
 */
public class Bot extends TelegramLongPollingBot {
    ...
    public String getBotToken() {
        return "6518988683:AAG6V8dNQMDKxnU8aUn0zyoyF4h6r0M-IgI";
    }
    ...
    private SendMessage runCommonCommand(Message message) throws InvocationTargetException, IllegalAccessException {
        ...
        Method[] classMethods =  commands.getClass().getDeclaredMethods(); // что такое инстанс ?
            for (Method method: classMethods) {
                if(method.isAnnotationPresent(AppBotCommand.class)){
                    AppBotCommand command  = method.getAnnotation(AppBotCommand.class);
                    if(command.name().equals(text)){
                        method.setAccessible(true);  // делает метод доступным
                        String responseText = (String) method.invoke(commands); // возвращение метода
   ...
   private Object runPhotoFilter(Message newMessage){
    ...
    private SendMediaGroup preparePhotoMessage(List<String> localPaths, ImageOperation operation, String chatId) throws Exception {
        ...
        for (String path: localPaths) {
            ...
            inputMedia.setMedia(new File(path),"media" + path);
